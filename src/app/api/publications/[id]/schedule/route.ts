...
import { schedulePublication } from "@/lib/services/publication-service";
import { parseSchedulePublicationInput } from "@/lib/validators/publication";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const { scheduledFor } = parseSchedulePublicationInput(await request.json());
    return NextResponse.json({ ok: true, data: await schedulePublication(supabase, (await context.params).id, scheduledFor) });
